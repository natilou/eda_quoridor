from re import A
from board.adapter_board import AdapterBoard
from domains.move import Move
...
class BoardExpert:
    ...
    def check_available_moves(board, side) -> list[Move]:
        ...
        for (row, col) in my_pawns_positions:
            ...
            # move right side
            if (row+move_forward, col) in cells_with_walls and (row, col+1) not in cells_with_walls and (row, col+1) not in opponent_pawns_positions:
                all_available_moves.append(
